...
DAG_ID = "conciliacao_newcorban_att_parceiro"

ATT_TABLE = "newcorban_att_parceiro"
...
"""
    ...
    sql_propostas = f"""
        ...
        WHERE bank_proposal_number IS NOT NULL and plataforma ilike 'PARCEIRO%' and status_id not in (9115 , 9117)
    """
...
def gerar_conciliacao_att_parceiro(**context):
    ...
    logger.info("[CONCILIACAO] Arquivo de saida do parceiro: %s", output_path)
...
with DAG(
    ...
    catchup=False,
    max_active_runs=1,
    description="Concilia fases do attParceiro com propostas incrementais NewCorban e exporta XLSX.",
    tags=["newcorban", "att-parceiro", "conciliacao", "excel"],
) as dag:
    gerar_conciliacao_task = PythonOperator(
        task_id="gerar_conciliacao_att_parceiro",
        python_callable=gerar_conciliacao_att_parceiro,
    )
